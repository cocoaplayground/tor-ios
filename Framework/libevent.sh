...
do
    ...
    mkdir -p $PREFIX
    ./configure --disable-shared --enable-static --disable-debug-mode --host=${HOST} --prefix=${PREFIX} CFLAGS="${OTHER_CFLAGS} -I${XPC_INCLUDE_DIR} -arch ${ARCH}" CPPFLAGS="${OTHER_CFLAGS} -I${XPC_INCLUDE_DIR} -arch ${ARCH}" LDFLAGS="${OTHER_LDFLAGS}"
...
xcrun --sdk $PLATFORM_NAME lipo -create "${BUILT_PRODUCTS_DIR}/libevent_openssl."*.a -output "${BUILT_PRODUCTS_DIR}/libevent_openssl.a"
