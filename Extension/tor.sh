...
do
    ...
    ./configure --disable-tool-name-check --enable-static-openssl --enable-static-libevent --disable-asciidoc --host $HOST --disable-gcc-hardening --disable-linker-hardening --prefix "${CONFIGURATION_TEMP_DIR}/tor-${ARCH}" --with-libevent-dir="${CONFIGURATION_TEMP_DIR}/libevent-${ARCH}" --with-openssl-dir="${CONFIGURATION_TEMP_DIR}/openssl-${ARCH}" CC="xcrun --sdk ${PLATFORM_NAME} clang -arch ${ARCH}" CFLAGS="${OTHER_CFLAGS} -I${PSEUDO_SYS_INCLUDE_DIR} -isysroot ${SDKROOT}" CPPLAGS="${OTHER_CFLAGS} -I${PSEUDO_SYS_INCLUDE_DIR} -isysroot ${SDKROOT}" ac_cv_func__NSGetEnviron="no" LDFLAGS="${OTHER_LDFLAGS}"
